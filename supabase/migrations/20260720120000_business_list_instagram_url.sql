...
as $$
  ...
  enriched as (
    select
      ...
      coalesce(
        latest_audit.detections -> 'instagram' -> 'evidence' -> 'links' ->> 0,
        case
          when coalesce(b.raw ->> 'socialPlatform', b.raw ->> 'social_platform', '') ilike '%instagram%'
            then b.raw ->> 'websiteUri'
        end,
        case
          when b.raw ->> 'websiteUri' ilike '%instagram.com%' then b.raw ->> 'websiteUri'
        end
      ) as instagram_url,
