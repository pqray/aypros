-- Phase 08 follow-up: expose social_only (site badge needs 4 states per specs/11:
-- sem site / social apenas / URL / fora do ar). Changing the return column list
-- requires dropping the function first (Postgres rejects RETURNS TABLE changes
-- on CREATE OR REPLACE).

drop function if exists public.get_org_businesses(
  uuid, text, integer, integer, numeric, boolean, boolean, boolean, uuid[], text, text, text, integer, integer
);

create or replace function public.get_org_businesses(
  org_id uuid,
...
as $$
  ...
  where public.is_org_member(org_id)
    and (business_ids is null or e.business_id = any (business_ids))
