-- Fase 20 (P1): estimador de custo e proposta na tela de oportunidade.
alter table public.leads
  add column if not exists domain_cost_annual numeric not null default 40,
  add column if not exists hosting_cost_monthly numeric not null default 0,
  add column if not exists margin_target_percent numeric null,
  add column if not exists estimated_monthly_cost numeric null,
  add column if not exists suggested_maintenance_value numeric null;

-- create or replace não permite mudar o tipo de retorno (novas colunas de
-- custo); precisa dropar antes de recriar.
