-- get_business_audit_summary é chamado pela API via service role (audits.ts),
-- depois de requireOrgContext já ter validado que o usuário pertence ao
-- org_id passado. Mas a função checava is_org_member(org_id), que depende de
-- auth.uid() — sempre null numa chamada com a service role key, então a
-- checagem falhava pra toda chamada e a rota caía sempre no fallback lento
-- (2 round-trips extras não medidos, ~350-400ms a mais por request).
-- Fix: aceitar a chamada quando vem da service role (já confiável, veio do
-- nosso backend) OU quando o próprio usuário autenticado é membro do org
-- (continua protegendo uma chamada direta via PostgREST com org_id alheio).
...
as $$
  with authorized_business as (
    ...
    where b.id = target_business_id
      and (auth.role() = 'service_role' or public.is_org_member(org_id))
      and exists (
        select 1
